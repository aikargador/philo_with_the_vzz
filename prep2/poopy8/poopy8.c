...
int	death_reaper(t_pp *ppp)
{
	...
	sploop = ((ppp->poopy.tv_sec * 1000 + ppp->poopy.tv_usec / 1000) % 1000)
		- ((ppp->froopy.tv_sec - ppp->froopy.tv_usec / 1000) % 1000);
	if (sploop < 0)
		sploop += 1000;
	printf("value of sploop -> %ld\n", sploop);
	printf("value of time to die -> %d\n", ppp->info->tT_DIE);
