...
int	death_reaper(t_philo *drPhil)
{
	...
	curntTime = (drPhil->curntTime.tv_sec * 1000 + \
	drPhil->curntTime.tv_usec / 1000) % drPhil->table->info.fig;
	prevTime = (drPhil->initTime.tv_sec * 1000 + \
	drPhil->initTime.tv_usec / 1000) % drPhil->table->info.fig;
