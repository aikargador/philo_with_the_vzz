...
void	*thrawd(void *agu) {
	...
	// }
	// if (sloopy->id % 2) {
	// 	printf("thread no. %d picked prosthetic arm no. %d\n", sloopy->id, sloopy->the_hand->hand);
	...
	if (sloopy->id % 2)
	{
		if (pthread_mutex_lock(&sloopy->the_neighbours_hand->cuffs) == EBUSY)
			printf("thread no. %d -> waiting\n", sloopy->id);
		else
			printf("thread no. %d picked prosthetic arm no. %d\n", sloopy->id, sloopy->the_neighbours_hand->hand);
		if (pthread_mutex_lock(&sloopy->the_hand->cuffs) == EBUSY)// to see the effect of the mutex lock comment the mutex lock and unlocks and test it
			printf("thread no. %d -> waiting\n", sloopy->id);
		else
			printf("thread no. %d picked prosthetic arm no. %d\n", sloopy->id, sloopy->the_hand->hand);
		printf("hello\n");
